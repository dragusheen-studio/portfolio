...
function FeaturedCard({ project }: FeaturedCardProps) {
    ...
    return (
        <div
            className="relative w-full max-w-lg mx-auto bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl transition-transform duration-500 hover:scale-[1.02] group cursor-pointer"
        ...
        >
            ...
            <div className="p-8 flex flex-col items-start relative z-20 -mt-6">
                ...
                <div className="flex w-full gap-3 pt-4 border-t border-white/5 mt-auto">
                    <Button variant="primary" className="flex-1 text-xs py-2" onClick={(e) => { e.stopPropagation(); navigate(`/studio/${project.id}`); }}>
                        Découvrir
                    </Button>
                </div>
            </div>
